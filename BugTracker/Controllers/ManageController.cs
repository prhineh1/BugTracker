...
namespace BugTracker.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ChangePassword(ChangePasswordAndNameViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ChangeName (ChangePasswordAndNameViewModel model)
        {
            var userId = User.Identity.GetUserId();
            var currentUser = db.Users.FirstOrDefault(u => u.Id == userId);

            //check for duplicate names
            foreach (var name in db.Users.Where(u => u.Id != userId))
            {
                if (name.FirstName.ToLower().Trim() == model.firstName.ToLower().Trim() && name.LastName.ToLower().Trim() == model.lastName.ToLower().Trim())
                {
                    TempData["warn"] = "copy";
                    return View("ChangePassword", model);
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ChangePic (HttpPostedFileBase profilePic)
        {
            var userId = User.Identity.GetUserId();
            var currentUser = db.Users.FirstOrDefault(u => u.Id == userId);
            ...
            if (ImageUploadValidator.IsWebFriendlyImage(profilePic))
            {
                ...
                try
                {
                    ...
                    foreach (var img in Directory.GetFiles(Path.Combine(Server.MapPath("~/Uploads/"))))
                    {
                        ...
                        {
                            currentUser.profilePic = "/Uploads/" + Path.GetFileName(profilePic.FileName);
                            notStored = false;
                            break;
                        }
                    }
                ...
                if (notStored)
                {
                    ...
                    currentUser.profilePic = "/Uploads/" + completeName;
                }
            }
